...
// 0 == main menu, 1 == game screen, 2 == game over screen
int gameState = 0;
...
void level::drawLevel() {
    std::ostringstream sout;
    sout << "Level: " << level;
    DrawText(sout.str().c_str(), 105 * fontScale, fontScale, 3.5 * fontScale, WHITE);
};
...
static void UpdateDrawFrame(void) {
    // main menu screen
    if (gameState == 0) {
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT)) { gameState = 1; }

        BeginDrawing();

            ClearBackground(BLACK);
            int textWidth = MeasureText("Tetris", 10 * fontScale);
            DrawText("Tetris", screenWidth/2 - textWidth/2, screenHeight/2 - 7.5 * fontScale, 10 * fontScale, WHITE);

        EndDrawing();

    } else if (gameState == 1) { // game screen
        // delta time variable
        grav += GetFrameTime();
        dt += GetFrameTime();
